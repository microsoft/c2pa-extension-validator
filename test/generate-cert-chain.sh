...
openssl req -new -newkey ec:<(openssl ecparam -name prime256v1) -keyout "trusted/soon_expired.key" -out "trusted/soon_expired.csr" -nodes -subj "/O=C2PA Extension Validator/OU=Test/CN=Test Expired Signer" -config openssl_ca.cnf -extensions v3_signer -sha256
openssl x509 -req -in "trusted/soon_expired.csr" -out "trusted/soon_expired.crt" -CA "trusted/CA.crt" -CAkey "trusted/CA.key" -CAcreateserial -days 1 -extfile openssl_ca.cnf -extensions v3_signer -sha384
